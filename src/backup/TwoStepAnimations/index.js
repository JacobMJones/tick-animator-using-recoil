...
function TwoStepAnimations({random, cr }) {
    ...
    const switcher = () => {
        if (internalCount === animation.length) {
            ...
            switch (animation.id) {
                case 1:
                    setAnimation(creature.actions[animation.id])
                    setInternalCount(1)
                    setNextAnimation(creature.actions[animation.id+1])
                    return;
                case 3:
                    setAnimation(creature.actions[1])
    ...
    useEffect(() => {
        ...
        creature && random && doRandom(random.num) === 1 && clickHandler()
...
export default TwoStepAnimations;
